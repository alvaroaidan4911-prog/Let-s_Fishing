...
(function(){
  ...
  function buildPanel(){
    ...
    Object.assign(fcJoy.style,{
      ...
      background:"rgba(0,0,0,0.5)", border:"2px solid rgba(200,100,255,0.5)",
    ...
    Object.assign(fcStick.style,{
      ...
      background:"rgba(200,100,255,0.8)",position:"absolute",
      left:"37px",top:"37px",pointerEvents:"none"
    });
    ...
    fcLbl.textContent = "gerak kamera";
    ...
    fcJoy.addEventListener("touchmove", function(e){
      ...
      fcStick.style.left = (37+cx)+"px";
      fcStick.style.top  = (37+cy)+"px";
    ...
    fcJoy.addEventListener("touchend", function(){
      fcJoyActive=false;
      fcStick.style.left="37px"; fcStick.style.top="37px";
  ...
  function startCinematic(){
    ...
    // Set kamera ke shot yang dipilih
    var s = SHOTS.find(function(x){return x.id===cinShot;});
    if(s){ cinDist = s.dist; cinPitch = s.pitch; }
    cinYaw = window.camYaw || 0;
    cinFreeCamPos = null; // reset free cam
    cinTarget.copy(player.position);
    cinTarget.y += 3.3;
    if(cinShot === 3) cinOrbit = true;
    if(cinShot === 6) cinFreeCam = true; else cinFreeCam = false;
    // Sembunyikan UI & player
    hideGameUI();
    // Show free cam joystick if needed
    var fj = document.getElementById("cinFreeJoy");
    var fu = document.getElementById("cinFreeUD");
    if(fj) fj.style.display = cinFreeCam?"flex":"none";
    if(fu) fu.style.display = cinFreeCam?"flex":"none";
    document.getElementById("cinWatermark").style.display = "block";
    var btn = document.getElementById("cinStartBtn");
    if(btn){ btn.textContent = "⏹ Hentikan Cinematic"; btn.style.background = "linear-gradient(135deg,#c0392b,#e74c3c)"; }
  ...
  document.addEventListener("touchmove", function(e){
    if(!cinActive) return;
    var panel = document.getElementById("cinPanel");
    if(panel && panel.style.display !== "none") return; // jangan rotate saat panel terbuka

    if(e.touches.length === 1){
      var dx = e.touches[0].clientX - touchStartX;
      var dy = e.touches[0].clientY - touchStartY;
      if(cinFreeCam){
        // Free cam: swipe = look around
        cinFreeCamYaw   -= dx * 0.004;
        cinFreeCamPitch  = Math.max(-1.3, Math.min(1.3, cinFreeCamPitch + dy * 0.003));
      } else {
        // Shot modes: swipe = orbit yaw/pitch
        cinYaw   += dx * 0.005;
    ...
    // Pinch = zoom (shot) or speed (free cam)
    if(e.touches.length === 2){
      var dx2 = e.touches[0].clientX - e.touches[1].clientX;
      var dy2 = e.touches[0].clientY - e.touches[1].clientY;
      var d = Math.sqrt(dx2*dx2+dy2*dy2);
      var delta = (d - pinchStartDist) * 0.1;
      if(cinFreeCam){
        cinFreeCamSpeed = Math.max(0.05, Math.min(3, cinFreeCamSpeed + delta * 0.01));
      } else {
