...
(function(){
  ...
  function buildPanel(){
    ...
    hint.textContent = "Saat cinematic aktif: geser layar = putar kamera | zoom = +/- jarak";
  ...
  function startCinematic(){
    ...
    hidePanel();
    showMessage("🎬 Cinematic aktif! Tap 🎬 untuk buka panel");
  }
  ...
  window._cinUpdateCamera = function(){
    ...
    // Touch drag for camera pan (saat panel tidak terbuka)
  ...
  // Touch swipe to rotate camera in cinematic
  ...
  document.addEventListener("touchmove", function(e){
    ...
    if(e.touches.length === 2){
      // Pinch zoom
      cinDist = Math.max(2, Math.min(120, cinDist));
    }
  }, {passive:true});
  ...
  window.addEventListener("keydown", function(e){
    ...
    if(k>="1"&&k<="5"){ selectShot(parseInt(k)); var s=SHOTS.find(function(x){return x.id===cinShot;}); if(s){cinDist=s.dist;cinPitch=s.pitch;} }
