...
// settingsBack dipanggil tombol ← di HTML
...
(function(){
  ...
  function toggleCinematic(){
    if(!isOwner()){ showMessage("❌ Hanya Owner yang bisa pakai Cinematic Mode!"); return; }
  ...
  // ── Init: tunggu game start lalu buat UI (owner only) ──
  const _cinInitInterval = setInterval(()=>{
    if(window.gameStarted && window.player){
      clearInterval(_cinInitInterval);
      if(!isOwner()) return; // hanya owner
      buildUI();
      window.cinematicToggle = toggleCinematic;

      // Tombol floating 🎬 untuk Android — tidak perlu keyboard
      ...
      fab.addEventListener("click", toggleCinematic);
