...
(function buildFishIndex(){
  ...
  Object.assign(btn.style,{
    position:"fixed",right:"12px",bottom:"160px",transition:"all 0.3s",
...
window.addEventListener("keydown",e=>{
  ...
  if(k==="c"&&localStorage.getItem("playerName")==="Varz444"){if(window.cinematicToggle)window.cinematicToggle();}
...
function animate(time){
  ...
  if(window.cinActive){window.updateCinematicCamera();}else{updateCamera();}
...
function adaptUI(){
  ...
  if(lnd){
    ...
    if(fibtn){Object.assign(fibtn.style,{bottom:"120px",right:"6px",width:"32px",height:"32px",fontSize:"14px"});}
    ...
    if(fibtn){Object.assign(fibtn.style,{bottom:"160px",right:"12px",width:"40px",height:"40px",fontSize:"18px"});}
...
// ═══════════════════════════════════════════════════════════
// 🎬 CINEMATIC MODE — Owner Only
// Aktif hanya jika playerName === OWNER_NAME
// Kontrol:
//   C          → Toggle cinematic mode on/off
//   1-5        → Pilih shot preset (drone, close-up, orbit, wide, follow)
//   Arrow Keys → Free camera pan (saat cinematic aktif)
//   Q/E        → Camera up/down
//   R          → Toggle auto-orbit
//   T          → Toggle DOF vignette overlay
//   F9         → Screenshot (download PNG)
// ═══════════════════════════════════════════════════════════
(function(){
  // ── Owner check ──
  const OWNER = "Varz444";
  ...
  // ── State ──
  window.cinActive = false;
  let cinActive = false; // local shadow
  let cinShot     = 1;          // 1=Drone 2=CloseUp 3=Orbit 4=Wide 5=Follow
  let cinOrbit    = false;
  let cinOrbitAngle = 0;
  let cinVignette = false;
  let cinFreeYaw  = 0, cinFreePitch = 0.4;
  let cinFreeDist = 18;
  let cinLookTarget = new THREE.Vector3();
  let cinKeys     = {};
  let cinUI       = null;

  // Shot definitions: { dist, pitch, label, emoji }
  const SHOTS = {
    1:{ dist:55,  pitch:1.1,  label:"Drone",    emoji:"🚁", desc:"High aerial overview" },
    2:{ dist:4,   pitch:0.15, label:"Close-Up",  emoji:"🎥", desc:"Dramatic close shot" },
    3:{ dist:22,  pitch:0.45, label:"Orbit",     emoji:"🔄", desc:"Orbiting around player" },
    4:{ dist:85,  pitch:0.85, label:"Wide",      emoji:"🌅", desc:"Epic wide angle" },
    5:{ dist:14,  pitch:0.28, label:"Follow",    emoji:"🎬", desc:"Cinematic follow cam" },
  };

  // ── Build UI ──
  function buildUI(){
    cinUI = document.createElement("div");
    cinUI.id = "cinematicUI";
    Object.assign(cinUI.style, {
      position:"fixed", top:"50%", left:"50%",
      transform:"translate(-50%,-50%) translateY(-160px)",
      background:"linear-gradient(135deg,rgba(0,0,0,0.92),rgba(10,10,30,0.95))",
      border:"1.5px solid rgba(255,200,50,0.5)",
      borderRadius:"16px", padding:"16px 20px",
      color:"#fff", fontFamily:"Arial", zIndex:"9998",
      display:"none", minWidth:"300px",
    ...
    cinUI.innerHTML = `
      <div style="display:flex;align-items:center;gap:8px;margin-bottom:12px;">
        ...
        <span style="font-weight:bold;font-size:15px;color:#ffe066;">CINEMATIC MODE</span>
        <span id="cinRecDot" style="margin-left:auto;width:10px;height:10px;border-radius:50%;background:#e74c3c;animation:cinBlink 1s infinite;"></span>
      </div>
      <div id="cinShotLabel" style="font-size:13px;color:#aaa;margin-bottom:10px;text-align:center;"></div>
      <div style="display:flex;gap:6px;margin-bottom:12px;" id="cinShotBtns"></div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:11px;color:#aaa;" id="cinControls">
        <div>↑↓←→ Pan kamera</div><div>Q/E Naik/turun</div>
        <div>[R] Auto-orbit: <span id="cinOrbitLbl">OFF</span></div>
        <div>[T] Vignette: <span id="cinVignetteLbl">OFF</span></div>
        <div>[F9] Screenshot</div><div>[C] Keluar</div>
      </div>
      <div style="margin-top:10px;font-size:10px;color:#888;text-align:center;">Shots [1-5] • Owner Only 👑</div>
    `;

    // Shot buttons
    const btnRow = cinUI.querySelector("#cinShotBtns");
    for(let i=1;i<=5;i++){
      const b = document.createElement("button");
      b.id = "cinShot"+i;
      b.title = SHOTS[i].label+" — "+SHOTS[i].desc;
      b.textContent = SHOTS[i].emoji;
      Object.assign(b.style,{
        flex:"1", padding:"7px 4px", border:"1.5px solid rgba(255,255,255,0.15)",
        borderRadius:"8px", background:"rgba(255,255,255,0.07)",
        color:"#fff", fontSize:"16px", cursor:"pointer",
        transition:"all 0.15s"
      });
      b.onclick = ()=>{ cinShot=i; applyShot(i); updateUI(); };
    ...
    style.textContent = `@keyframes cinBlink{0%,100%{opacity:1}50%{opacity:0.2}}
    #cinematicUI button:hover{background:rgba(255,200,50,0.2)!important;border-color:rgba(255,200,50,0.5)!important;}`;
    document.head.appendChild(style);

    // Watermark saat cinematic
    ...
    Object.assign(wm.style,{
      ...
      textShadow:"0 1px 4px rgba(0,0,0,0.8)", letterSpacing:"1px"
    });
    wm.textContent = "🎣 Let's Fishing!";
    document.body.appendChild(wm);

    document.body.appendChild(cinUI);
  }

  function updateUI(){
    if(!cinUI) return;
    const shot = SHOTS[cinShot];
    cinUI.querySelector("#cinShotLabel").textContent =
      shot.emoji+" "+shot.label+" — "+shot.desc;
    for(let i=1;i<=5;i++){
      const b = document.getElementById("cinShot"+i);
      if(b) b.style.background = i===cinShot
        ? "rgba(255,200,50,0.3)" : "rgba(255,255,255,0.07)";
    }
    const ol = document.getElementById("cinOrbitLbl");
    if(ol) ol.textContent = cinOrbit?"ON 🔄":"OFF";
    if(ol) ol.style.color = cinOrbit?"#f1c40f":"#888";
    const vl = document.getElementById("cinVignetteLbl");
    if(vl) vl.textContent = cinVignette?"ON":"OFF";
    if(vl) vl.style.color = cinVignette?"#f1c40f":"#888";
  }

  // ── Apply shot preset ──
  function applyShot(n){
    const s = SHOTS[n];
    cinFreeDist  = s.dist;
    cinFreePitch = s.pitch;
    cinShot = n;
    // Auto-enable orbit only for shot 3
    if(n===3){ cinOrbit=true; }
    showMessage("🎬 Shot: "+s.emoji+" "+s.label);
  }

  // ── Toggle cinematic ──
  function toggleCinematic(){
    if(!isOwner()){ if(window.showMessage)showMessage("❌ Hanya Owner yang bisa pakai Cinematic Mode!"); return; }
    if(!cinUI){ buildUI(); }
    cinActive = !cinActive; window.cinActive = cinActive;
    cinUI.style.display = cinActive?"block":"none";
    document.getElementById("cinVignette").style.display = cinActive&&cinVignette?"block":"none";
    document.getElementById("cinWatermark").style.display = cinActive?"block":"none";

    if(cinActive){
      // Sync initial yaw/pitch from current camera
      cinFreeYaw   = window.camYaw   || 0;
      cinFreePitch = window.camPitch || 0.4;
      cinFreeDist  = SHOTS[cinShot].dist;
      cinLookTarget.copy(player.position);
      cinLookTarget.y += 3.3;
      applyShot(cinShot);
      updateUI();
      showMessage("🎬 Cinematic ON — [C] keluar | [1-5] shots");
    } else {
      cinOrbit  = false;
      showMessage("🎬 Cinematic OFF");
    }
  }

  // ── Cinematic camera update (called from animate loop) ──
  window.updateCinematicCamera = function(){
    if(!cinActive) return;

    const spd = 0.04;
    // Free cam pan with arrow keys
    if(cinKeys["arrowleft"])  cinFreeYaw   -= spd;
    if(cinKeys["arrowright"]) cinFreeYaw   += spd;
    if(cinKeys["arrowup"])    cinFreePitch  = Math.max(0.05, cinFreePitch - spd*0.5);
    if(cinKeys["arrowdown"])  cinFreePitch  = Math.min(1.45, cinFreePitch + spd*0.5);
    if(cinKeys["q"])          cinFreeDist   = Math.max(2, cinFreeDist - 0.3);
    if(cinKeys["e"])          cinFreeDist   = Math.min(120, cinFreeDist + 0.3);

    // Auto orbit
    if(cinOrbit) cinFreeYaw += 0.004;

    // Smooth look target → player
    const wp = new THREE.Vector3();
    player.getWorldPosition(wp);
    wp.y += 3.3;
    cinLookTarget.lerp(wp, cinShot===2?0.04:0.08);

    // Compute camera position
    const camX = cinLookTarget.x - Math.sin(cinFreeYaw)*cinFreeDist;
    const camY = cinLookTarget.y + cinFreePitch * (cinFreeDist*0.38);
    const camZ = cinLookTarget.z - Math.cos(cinFreeYaw)*cinFreeDist;

    // Shot 5 (Follow) — camera lags behind player movement
    const lerpSpeed = cinShot===5 ? 0.05 : cinShot===2 ? 0.07 : 0.12;
    camera.position.lerp(new THREE.Vector3(camX,camY,camZ), lerpSpeed);
    camera.lookAt(cinLookTarget);
  ...
  function takeScreenshot(){
    // Hide UI temporarily
    const uiEls = ["cinematicUI","coinUI","levelUI","weatherUI","joystick",
                   "hotbar","runBtn","inventoryBtn","fishIndexBtn","dayNightUI",
                   "fpsCounter","islandBadge","mpStatusBadge","ownerPanelBtn"].map(
    ...
    showMessage("📸 Screenshot disimpan!");
  }

  // ── Keyboard handling ──
  window.addEventListener("keydown",e=>{
    cinKeys[e.key.toLowerCase()] = true;
    if(!window.gameStarted) return;
    const k = e.key.toLowerCase();
    // C → toggle cinematic (owner only)
    if(k==="c" && isOwner()) toggleCinematic();
    if(!cinActive) return;
    // Number keys 1-5 → shot
    if(k>="1"&&k<="5"){ cinShot=parseInt(k); applyShot(cinShot); updateUI(); }
    // R → orbit toggle
    if(k==="r"){ cinOrbit=!cinOrbit; updateUI(); }
    // T → vignette toggle
    if(k==="t"){
      cinVignette=!cinVignette;
      document.getElementById("cinVignette").style.display=cinVignette?"block":"none";
      updateUI();
    }
    // F9 → screenshot
    if(e.key==="F9"){ e.preventDefault(); takeScreenshot(); }
  });
  window.addEventListener("keyup",e=>{ cinKeys[e.key.toLowerCase()]=false; });

  // ── Wait for game to start then build UI ──
  window.addEventListener("load",()=>{
    const t = setInterval(()=>{
      if((window.gameStarted || gameStarted) && window.player && window.camera){
        clearInterval(t);
        buildUI();
        // Add cinematic button to owner panel if exists
        // (owner panel trigger via multiplayer.js)
        window.cinematicToggle = toggleCinematic;
      }
    }, 500);
  });

  // ── Expose for animate loop hook ──
  // Patch updateCamera to allow cinematic override
  const _origUpdateCamera = window.updateCamera || function(){};
  window.updateCamera = function(){
    if(window.cinActive){ window.updateCinematicCamera(); return; }
    _origUpdateCamera();
  };
