...
function buildIsland(def,options){
  ...
  g.add(new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({map:sandTex,roughness:0.9})
  ));
  // Grass flat layer (Y=2.5 relative = Y=0 world)
  const gr=new THREE.Mesh(
    new THREE.CylinderGeometry(grassR,grassR+5,0.55,48),
...
// Build all islands
buildIsland(islandDefs[0],{trees:16,rocks:8,flowers:24,grassColor:0x2e9e2e,trunkColor:0x8B6914,leafColor:0x1a8a1a,lampColor:0xffdd88,paths:true,benches:true,lamps:true,lampCount:5,labelColor:"#ffdd88"});
